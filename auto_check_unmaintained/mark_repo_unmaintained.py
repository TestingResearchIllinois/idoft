...
def update_row(row):
    if row['Project URL'] in last_commit_dict:
        if pd.isna(row['Status']):  # Check if 'Status' is empty
            row['Status'] = 'Unmaintained'
        else:
            return row

        row['Notes'] = f"{row['Notes']} | {last_commit_dict[row['Project URL']]}" if pd.notna(row['Notes']) else f"{last_commit_dict[row['Project URL']]}"
    return row
